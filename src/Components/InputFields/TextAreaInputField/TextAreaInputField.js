...
function TextAreaInputField(props) {
  ...
  const inputClassName = (error) 
                         ? `${props.inputClassName} paragraph2 TextArea-inputField TextArea-inputFieldError` 
  ...
  return (
    <div className={className}>
      ...
      <textarea type={props.type} name={props.name} id={props.id} className={inputClassName} value={props.value} onChange={handleInputChange}/>
  </div>
  )
}
